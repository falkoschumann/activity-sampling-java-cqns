...
public class CsvEventStoreTests {
  ...
  void replay() throws Exception {
    ...
    var expected = createEvents();
    var actual = events.collect(Collectors.toList());
    assertEquals(expected.size(), actual.size(), "Size");
    assertEquals(expected.get(0).tags().size(), actual.get(0).tags().size(), "Index 0 - Tags");
    assertEquals(expected.get(0), actual.get(0), "Index 0");
    assertEquals(expected.get(1), actual.get(1), "Index 1");
  }
