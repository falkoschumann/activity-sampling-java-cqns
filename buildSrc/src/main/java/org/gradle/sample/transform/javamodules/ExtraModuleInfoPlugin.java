...
public class ExtraModuleInfoPlugin implements Plugin<Project> {
  ...
  public void apply(Project project) {
    ...
    ExtraModuleInfoPluginExtension extension =
        project.getObjects().newInstance(ExtraModuleInfoPluginExtension.class);
    project
        .getExtensions()
        .add(ExtraModuleInfoPluginExtension.class, "extraJavaModuleInfo", extension);
  ...
  private void configureTransform(Project project, ExtraModuleInfoPluginExtension extension) {
    ...
    project
        .getConfigurations()
        .matching(this::isResolvingJavaPluginConfiguration)
        .all(c -> c.getAttributes().attribute(javaModule, true));

    // all Jars have a javaModule=false attribute by default; the transform also recognizes modules
    ...
    project
        .getDependencies()
        .registerTransform(
            ExtraModuleInfoTransform.class,
            t -> {
              t.parameters(
                  p -> {
  ...
  private boolean isResolvingJavaPluginConfiguration(Configuration configuration) {
    ...
    return configuration
            .getName()
            .endsWith(JavaPlugin.COMPILE_CLASSPATH_CONFIGURATION_NAME.substring(1))
        || configuration
            .getName()
            .endsWith(JavaPlugin.RUNTIME_CLASSPATH_CONFIGURATION_NAME.substring(1))
        || configuration
            .getName()
            .endsWith(JavaPlugin.ANNOTATION_PROCESSOR_CONFIGURATION_NAME.substring(1));
  }
}
